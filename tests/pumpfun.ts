...
import { Keypair, LAMPORTS_PER_SOL, PublicKey, SystemProgram, Transaction, TransactionConfirmationStrategy } from '@solana/web3.js';
...
import { calculateAmountOutBuy, convertFromFloat, convertToFloat, getAssociatedTokenAccount } from './utils';
...
describe('pumpfun', () => {
  ...
  before(async () => {
    ...
    const airdropTx = await connection.requestAirdrop(adminKp.publicKey, 5 * LAMPORTS_PER_SOL);
    ...
    const airdropTx2 = await connection.requestAirdrop(userKp.publicKey, 5 * LAMPORTS_PER_SOL);
    await connection.confirmTransaction({
    ...
    const airdropTx3 = await connection.requestAirdrop(user2Kp.publicKey, 5 * LAMPORTS_PER_SOL);
    await connection.confirmTransaction({
  ...
  it('Is correctly configured', async () => {
    ...
    const [configPda, _] = PublicKey.findProgramAddressSync([Buffer.from(SEED_CONFIG)], program.programId);
  ...
  it('Is the token created', async () => {
    ...
    const [bondingCurvePda, _] = PublicKey.findProgramAddressSync([Buffer.from(SEED_BONDING_CURVE), tokenKp.publicKey.toBytes()], program.programId);
  ...
  it("Is user1's simulate swap SOL for token completed", async () => {
    ...
    const adjustedAmount = convertFromFloat((convertToFloat(amount.toNumber(), TEST_DECIMALS) * (100 - configAccount.platformBuyFee)) / 100, TEST_DECIMALS);

    const amountOut = calculateAmountOutBuy(TEST_VIRTUAL_RESERVES, adjustedAmount, TEST_DECIMALS, TEST_TOKEN_SUPPLY);

    console.log('expected amount out: ', amountOut);
    console.log('actual amount out: ', actualAmountOut);
    assert.equal(actualAmountOut, +amountOut.toPrecision(actualAmountOut.toString().length - 1));
  });
  ...
  it("Is user1's swap Token for SOL completed", async () => {
    ...
    // Send the transaction to launch a token
    const tx = await program.methods
      .swap(new BN(23_000_000), 1, new BN(0))
      .accounts({
        teamWallet: configAccount.teamWallet,
        user: userKp.publicKey,
        tokenMint: tokenKp.publicKey
  ...
  it('Is the curve reached the limit', async () => {
    ...
    // Send the transaction to launch a token
    const tx = await program.methods
      .swap(new BN(3_000_000_000), 0, new BN(0))
      .accounts({
        teamWallet: configAccount.teamWallet,
    ...
    const [bondingCurvePda] = PublicKey.findProgramAddressSync([Buffer.from(SEED_BONDING_CURVE), tokenKp.publicKey.toBytes()], program.programId);
