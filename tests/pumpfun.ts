...
describe('pumpfun', () => {
  ...
  before(async () => {
    ...
    await connection.confirmTransaction({ signature: airdropTx, abortSignal: AbortSignal.timeout(1000) } as TransactionConfirmationStrategy);

    console.log('airdrop SOL to user');
    const airdropTx2 = await connection.requestAirdrop(userKp.publicKey, 5 * LAMPORTS_PER_SOL);
    await connection.confirmTransaction({ signature: airdropTx2, abortSignal: AbortSignal.timeout(1000) } as TransactionConfirmationStrategy);

    console.log('airdrop SOL to user2');
    const airdropTx3 = await connection.requestAirdrop(user2Kp.publicKey, 5 * LAMPORTS_PER_SOL);
    await connection.confirmTransaction({ signature: airdropTx3, abortSignal: AbortSignal.timeout(1000) } as TransactionConfirmationStrategy);
  });
